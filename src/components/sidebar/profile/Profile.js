        ...
        <>
            {
                ...
                <section className="p-5 overflow-y-scroll overflow-x-hidden no-scrollbar h-screen">       
                    ...
                    <div className="flex flex-col items-center w-full bg-[#36404A] p-3 rounded-lg relative overflow-hidden cursor-pointer group">
                        <span className="absolute top-0 right-0 p-2 bg-[#3E4A56] rounded-lg translate-x-[110%] group-hover:translate-x-0 cursor-pointer duration-300"
                            onClick={deleteAccountHandler}
                        >
                            <AiFillDelete size={25} />
                        </span>
                        <img src={user.image} className="w-[120px] h-[120px] rounded-full mb-3" alt="" />
                        <h1 className="mb-1 text-[20px]">{user.name}</h1>
                        <p className="text-[#ffffffa4]">{user.email}</p>
                    </div>
                    <div className="h-[.5px] w-full bg-white my-5" />
                    <section className="" >
                        <button onClick={showEditFormHandler} className="w-full mb-6">EDIT PROFILE</button>
                        {
                            isEditForm && 
                            <Form method="POST" className="p-3 bg-[#36404A] rounded-lg mb-5" encType="multipart/form-data">
                                <h1 className="text-center mb-4 text-[20px]">Change Your Profile</h1>
                                <div className="mb-3 text-[red] text-center text-[18px]">{error}</div>
                                { imagePreview &&
                                <div className="mb-1 bg-[#262E35] p-3 rounded-lg">
                                    <img src={imagePreview} className=" rounded-lg w-[100px] mx-auto" alt="" /> 
                                </div>
                                }
                                <Input 
                                    type="file"
